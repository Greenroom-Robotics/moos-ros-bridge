...
//      rclcpp::M_string header = event.getConnectionHeader();
 ...
 */
// int ROSNode::CreateROSPubSub(xml_node<> *node, vector<MsgContainer> *pubVec, vector<MsgContainer> *subVec)
...
// 	//   }
//      else{
// 	       RCLCPP_INFO( ,"ERROR PARSING XML CONFIG FILE\n");
// 	       return FAIL;
//      }
...
int main(int argc, char **argv)
{
     ...
     auto bridge_node = std::make_shared<ROSNode>("MOOS_ROS_Bridge");
     // Check the inputs and log result of checks.
     if(argc < 3){
          RCLCPP_INFO(bridge_node->get_logger(), "Invalid number of parameters\n\n");
          RCLCPP_INFO(bridge_node->get_logger(), "argc is %d, but it should be at least 3.\n", argc);
          RCLCPP_INFO(bridge_node->get_logger(), "Usage:\n\trosrun moosros Bridge <moosrosconfig.xml> <mission.moos>");
          return 0;
     }
     ...
     if(intStat != 0) {
          RCLCPP_INFO(bridge_node->get_logger(), "\n******\nCONFIG FILE MISSING\n%s does not exist!\n******\n",argv[1]);
          return 0;
     }
     ...
     vector<IMsgContainer> pubVec;

     //Get First Topic/Message
     xml_node<> *node = doc.first_node()->first_node();

     // RCLCPP_INFO(bridge_node->get_logger(),"'%s'", &node);

     if(bridge_node->CreateROSPubSub(node,&pubVec,&subVec) == FAIL)
          return 0;
